...
def findOpenClose(df):
    ...
    df_filtered = df[
        (df["Datetime"].dt.hour == 2) | (df["Datetime"].dt.hour == 15)]  # remove all but 2am (8:00) and 3pm (21:59)
    df_filtered = df_filtered.reset_index(drop=True)
...
def find08xxAnd09xx(df, minutesIn8xx, minutesIn9xx):
    ...
    df_filtered = df[
        ((df["Datetime"].dt.hour == 2) & (df["Datetime"].dt.minute == minutesIn8xx)) | (df["Datetime"].dt.hour == 3) & (
                    df["Datetime"].dt.minute == minutesIn9xx)]
    df_filtered = df_filtered.reset_index(drop=True)
...
def downloadDax_1h():
    ...
    with pd.ExcelWriter("C:/Users/FSX-P/Aktienanalyse/Indizes_src.xlsx", engine='openpyxl', mode='a',
        ...
        tickerNoTimezone.to_excel(writer, sheet_name='dax_data_temp', index=True)
...
def downloadDax_5m():
    ...
    with pd.ExcelWriter("C:/Users/FSX-P/Aktienanalyse/Indizes_src.xlsx", engine='openpyxl', mode='a',
                        if_sheet_exists="replace") as writer:
        tickerNoTimezone.to_excel(writer, sheet_name='dax_data_15m_temp', index=True)
...
def processFutureAnalyticsData():
    ...
    for i, day in df_joined.iterrows():
        ...
        df_result.at[i, "bool_isCloseValReachedBetween10am10pm"] = greaterEqual(val1000To2200High, close_minus1 * (
                    1 - percentBuffer * 0.01))
